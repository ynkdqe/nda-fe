...
const workspaceSummary = ref<WorkspaceSummaryResult['data']>();
...
const workspaceSummaryValues = computed(() => {
  const todos = workspaceSummary.value?.workspaceTodos;

  return {
    projectValue: workspaceSummary.value?.workspaceProjects?.total ?? '-',
    teamValue: workspaceSummary.value?.workspaceEmployees?.total ?? '-',
    todoValue: todos ? `${todos.completed ?? 0}/${todos.total ?? 0}` : '-',
  };
});
...
async function fetchWorkspaceSummary() {
  ...
  try {
    const response = await requestClient.get<WorkspaceSummaryResult>('/api/workspace', {
      responseReturn: 'body',
    });
    workspaceSummary.value = response.data;
  } catch (error) {
    console.error('Failed to fetch workspace summary:', error);
    workspaceSummary.value = undefined;
  } finally {
...
<template>
  <div class="p-5">
    <WorkbenchHeader
      ...
      :project-label="$t('page.dashboard.workspace.summary.projects')"
      :project-value="workspaceSummaryValues.projectValue"
      :team-label="$t('page.dashboard.workspace.summary.team')"
      :team-value="workspaceSummaryValues.teamValue"
      :todo-label="$t('page.dashboard.workspace.summary.todo')"
      :todo-value="workspaceSummaryValues.todoValue"
    >
      <template #title>
