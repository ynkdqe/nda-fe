...
export function calculateContractSalaryState({
  ...
  const hasSocialInsurance = isContractSocialInsuranceEnabled(
    contractType,
    gross,
  );
  ...
  const insuranceValue = hasSocialInsurance
    ? clamp(
        toNumber(form.insuranceValue, DEFAULT_INSURANCE_PERCENT),
        0,
        insuranceValueMax,
      )
    : 0;
  ...
  const businessSocialInsuranceFee = hasSocialInsurance
    ? calculatePercentFee(
        insuranceSalary,
        contractType?.businessSocialInsurancePercent,
      )
    : 0;
  const businessHealthInsuranceFee = hasSocialInsurance
    ? calculatePercentFee(
        insuranceSalary,
        contractType?.businessHealthInsurancePercent,
      )
    : 0;
  const businessUnemploymentInsuranceFee = hasSocialInsurance
    ? calculatePercentFee(
        insuranceSalary,
        contractType?.businessUnemploymentInsurancePercent,
      )
    : 0;
  const businessCalculateOccAccInsuranceFee = hasSocialInsurance
    ? calculatePercentFee(
        insuranceSalary,
        contractType?.businessOccAccInsurancePercent,
      )
    : 0;
